    ...
    <div className="fixed inset-0 z-50 grid place-items-center p-5">
      ...
      >
        <button
          onClick={onClose}
          aria-label="Close"
          className="group absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-[8px] text-ink-4 hover:bg-hover hover:text-ink"
        >
        ...
        <form action={action} className="mt-5">
          ...
          <input
            ...
            className="mt-1.5 w-full rounded-[10px] border border-line-strong bg-sunk px-3.5 py-2.5 font-mono text-[13px] text-ink outline-none transition-colors placeholder:font-sans placeholder:text-ink-4 focus:border-accent"
          />
          ...
          <button
            disabled={pending}
            className="mt-5 flex w-full items-center justify-center gap-2 rounded-[9px] bg-accent py-2.5 text-[14px] font-medium text-white transition-[filter] hover:brightness-110 disabled:opacity-60"
          >
            {pending ? (
              <>
