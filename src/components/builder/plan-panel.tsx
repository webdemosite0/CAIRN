    ...
    <div className="bezel overflow-hidden">
                ...
                <div className="flex flex-wrap gap-2">
                  {plan.style.palette.map((c) => (
                    <div key={c} className="text-center">
                      <span
                        className="block h-11 w-11 rounded-[9px] border border-line"
                        style={{ background: c }}
                      />
          ...
          <div className="space-y-3">
            ...
            {(
              ...
              <button
                ...
                className={cn(
                  "block w-full rounded-[10px] border p-3.5 text-left transition-colors",
                  storage === o.id
              ...
              >
                ...
                <span className="mt-2 flex flex-wrap gap-1.5">
                  {o.tags.map((t) => (
                    <span
                      key={t}
                      className="rounded-[6px] bg-sunk px-1.5 py-0.5 text-[11px] text-ink-4"
                    >
                      {t}
                    </span>
                  ))}
      ...
      <div className="border-t border-line bg-rail/60 p-3">
        ...
        <button
          onClick={onGenerate}
          disabled={busy}
          className="flex w-full items-center justify-center gap-2 rounded-[9px] bg-accent px-4 py-2.5 text-[14px] font-medium text-white shadow-[var(--elev)] transition-[filter,transform] duration-150 hover:brightness-110 active:scale-[0.99] disabled:opacity-50"
        >
          Generate App
        </button>
      </div>
